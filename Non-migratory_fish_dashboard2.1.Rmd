...
renderPlot({
  ...
  colour = "darkgrey") +
    geom_sf(data = origin.points(), colour = "red", size =5) +
    geom_sf(data = destination.points(), colour = "red", size = 5)+
...
leafletOutput('pathleaf', height=1000)
```
...
leaflet(options = leafletOptions(worldCopyJump = TRUE)) %>%
  ...
  addMarkers(data = my.start.end() %>% 
               st_transform(crs = 4326)) %>%
  addCircleMarkers(data = origin.points() %>% 
               st_transform(crs = 4326),
               color = "yellow") %>%
  addCircleMarkers(data = destination.points() %>% 
               st_transform(crs = 4326),
               color = "yellow") %>%
  addPolygons(data = final.catchments() %>% 
                st_transform(crs = 4326),
...
leafletOutput('interim', height=1000)
```
